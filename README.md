...
- 파일 첨부 미리보기 텍스트를 직접입력으로 다시 넣는 경우에도 `<<상품 정보>>`, `<<상세 일정>>`, `*1일차*` 구조를 보존해 파싱합니다. `<<상세 일정>>`이 없는 메타데이터-only 입력도 `상품 정보`, `항공/교통`, `숙박`, `포함/불포함`, `선택관광`, `쇼핑센터 방문 수`, `유의사항`을 반영하고, `기간` 기준으로 빈 일차를 생성합니다.

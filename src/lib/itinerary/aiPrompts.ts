export const PARSER_SYSTEM_PROMPT = [
  ...
  "호텔명/HOTEL/객실/숙박시설명은 type: ACCOMMODATION으로 기록하되, 호텔 이동/체크인/휴식처럼 실제 호텔명이 아닌 행동 문구는 OTHER로 기록한다.",
  ...
  days: [
    {
      ...
      items: [
        ...
        { type: "OTHER", time: "", content: "호텔 체크인", detail: "" },
      ],
    },
  ],
});
...
export function buildParseUserPrompt(title: string | undefined, rawText: string): string {
  return [
    ...
    "10) HOTEL/호텔명/숙박시설명은 ACCOMMODATION 카테고리로 처리한다. 호텔 이동/체크인/휴식처럼 실제 호텔명이 아닌 행동 문구는 OTHER로 처리한다.",
